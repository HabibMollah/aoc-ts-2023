const input2 = `mxmkjvgsdzfhseightonetwoeight7
...
function decode2(input: string) {
  ...
  const numberRegex = new RegExp(`\\d|${numberName.join('|')}`, 'g');

  const wordList = input.split('\n');
  return wordList
    .map((word) => {
      const numbersList = word.match(numberRegex) as string[];
      const firstNumber =
        numbersList[0].length > 1
          ? +numberName.indexOf(numbersList[0]) + 1
          : numbersList[0];
      const lastNumber =
        numbersList[numbersList.length - 1].length > 1
          ? +numberName.indexOf(numbersList[numbersList.length - 1]) + 1
...
console.log('decode2:', decode2(input2));
