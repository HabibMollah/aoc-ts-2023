const input = `mxmkjvgsdzfhseightonetwoeight7
...
/*
 ...
 - grab the last number
    - if no last number exists
        use the first number as last number
 - add first number and last number for each word
 - add all sum from all words
*/
function decode(input: string) {
  const wordList = input.split('\n');
  return wordList
    .map((word) => {
      const numbers = word.match(/\d/g) as string[];
